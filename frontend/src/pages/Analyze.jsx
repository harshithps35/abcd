    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // Fallback to client-side if backend is down
    ...
    <>
      ...
      <main className={styles.page}>
        ...
        {!uploadedImage ? (
          ...
          <>
            {/* Prominent Toss Recommendation Banner */}
            {analysis && (
              <div className={styles.tossRecommendation}>
                <div className={styles.tossGlow}></div>
                <div className={styles.tossContent}>
                  <div className={styles.tossIcon}>
                    {analysis.tossDecision === "Bowl First" ? "🎳" : "🏏"}
                  </div>
                  <div className={styles.tossInfo}>
                    <span className={styles.tossLabel}>TOSS RECOMMENDATION</span>
                    <span className={styles.tossDecision} style={{
                      color: analysis.tossDecision === "Bowl First" ? "var(--accent-green)" : "var(--accent-blue)"
                    }}>
                      {analysis.tossDecision}
                    </span>
                  </div>
                  <div className={styles.tossDivider}></div>
                  <div className={styles.tossReason}>
                    <p>
                      {analysis.tossDecision === "Bowl First"
                        ? `High moisture (${analysis.moisture}%) and grass cover (${analysis.grassCover}%) favor seam movement early on. Bowl first to exploit fresh conditions.`
                        : analysis.crackIndex > 45
                          ? `Significant cracks (${analysis.crackIndex}%) will worsen over time. Bat first and put runs on the board before the pitch deteriorates.`
                          : `Surface is true with good bounce (${analysis.bounce}%). Bat first to capitalize on the best batting conditions before wear sets in.`
                      }
                    </p>
                  </div>
                  <div className={styles.tossDominance}>
                    <span className={styles.tossDominanceLabel}>Pitch favors:</span>
                    <span className={styles.tossDominanceValue} style={{ color: analysis.dominanceColor }}>
                      {analysis.dominance}
                    </span>
                  </div>
                </div>
